...
public class UVCCameraHelper {
    ...
    public UVCCameraHelper(int cameraWidth, int cameraHeight) {
    ...
    public boolean updatePermission(final UsbDevice device, final boolean hasPermission) {
        return mUSBMonitor.updatePermission(device, hasPermission);
    }
    public void openCamera_StartPreview(final UsbDevice device)
    {
        mCtrlBlock = mUSBMonitor.getControlBlock(device);
